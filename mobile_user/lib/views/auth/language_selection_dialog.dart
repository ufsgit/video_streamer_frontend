...
class _LanguageSelectionDialogState extends State<LanguageSelectionDialog> {
  ...
  Future<void> _fetchLanguages() async {
    try {
      final response = await DioClient().dio.get(
        ApiConstants.userLanguagesPath,
      );
  ...
  void _saveAndContinue() async {
    ...
    final selectedLangId = _selectedLanguageId ?? 0;

    var box = Hive.box('settings');
    await box.put('selected_language', selectedLang);
    if (_selectedLanguageId != null) {
      await box.put('selected_language_id', _selectedLanguageId);
    }

    debugPrint(
      'DEBUG: Hive stored language -> ${box.get('selected_language')} (ID: ${box.get('selected_language_id')})',
    );

    // Send selected language to API user/profiles/language
    try {
      await ApiService().updateUserLanguage(
        languageId: selectedLangId,
        languageName: selectedLang,
      );
      debugPrint('DEBUG: Successfully sent language to API (ID: $selectedLangId, Name: $selectedLang)');
    } catch (e) {
      debugPrint('DEBUG: Error sending language to API: $e');
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                      ...
                      setState(() {
                        ...
                        if (value != null) {
                          ...
                          if (selectedObj.isNotEmpty) {
                            final rawId = selectedObj['id'] ??
                                selectedObj['language_id'] ??
                                selectedObj['_id'];
                            if (rawId != null) {
                              _selectedLanguageId = int.tryParse(rawId.toString());
                            }
                          }
                        }
                      });
            ...
            const SizedBox(height: 24),
            ElevatedButton(
              onPressed: _selectedLanguage == null || _isLoading || _isSaving
                  ? null
                  : _saveAndContinue,
