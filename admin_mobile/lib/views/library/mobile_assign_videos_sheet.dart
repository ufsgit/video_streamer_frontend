...
class _MobileAssignVideosSheetState extends State<MobileAssignVideosSheet> {
  ...
  // Library videos search & selection when opened from patient detail
  List<Map<String, dynamic>> _libraryVideos = [];
  ...
  void initState() {
    ...
    if (widget.preselectedPatient != null) {
      _selectedUser = {
        'id': widget.preselectedPatient!.id,
        'name': widget.preselectedPatient!.name,
        'username': widget.preselectedPatient!.username,
        'email': widget.preselectedPatient!.email,
        'phone': widget.preselectedPatient!.phone,
      };
    }
  ...
  Future<void> _fetchLibraryVideos() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _libraryVideos = raw.map((v) => Map<String, dynamic>.from(v as Map)).toList();
        });
      }
    } catch (e) {
      debugPrint("Error loading library videos: $e");
    } finally {
  ...
  Future<void> _assignVideos() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> newVideos = _currentVideos.map((v) => {
        ...
        'title': v['title'],
        'duration': v['duration'],
        'imageUrl': v['imageUrl'] ?? v['thumbnail_url'],
        'category': v['category'],
        'youtubeUrl': v['youtubeUrl'] ?? v['video_url'] ?? v['url'],
        'isCompleted': false,
              ...
              child: _currentVideos.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final img = video['imageUrl'] ?? video['thumbnail_url'] ?? '';
                        return Row(
                          children: [
                            ClipRRect(
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    "${video['duration'] ?? '10:00'} • ${video['category'] ?? 'General'}",
                                    style: const TextStyle(
                                      fontSize: 11,
                                      color: AppTheme.textSecondary,
                                    ),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setModalState) {
          final filtered = _libraryVideos.where((v) {
            if (_videoSearchQuery.isEmpty) return true;
            final t = (v['title'] ?? '').toString().toLowerCase();
            final c = (v['category'] ?? '').toString().toLowerCase();
            return t.contains(_videoSearchQuery.toLowerCase()) ||
                c.contains(_videoSearchQuery.toLowerCase());
          }).toList();
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: "Search library videos...",
                    prefixIcon: const Icon(Icons.search, size: 20),
                    filled: true,
                    fillColor: Colors.grey.shade50,
                ...
                Expanded(
                  child: _isLoadingVideos
                      ...
                      : filtered.isEmpty
                          ? const Center(child: Text("No videos found in library"))
                          : ListView.separated(
                              itemCount: filtered.length,
                              separatorBuilder: (context, i) => const Divider(height: 1),
                              itemBuilder: (context, index) {
                                final vid = filtered[index];
                                final isAdded = _currentVideos.any(
                                  (v) => v['id'] == vid['id'] || v['title'] == vid['title'],
                                );
                                ...
                                return CheckboxListTile(
                                  ...
                                  onChanged: (checked) {
                                    setModalState(() {
                                      if (checked == true) {
                                        _currentVideos.add(vid);
                                      } else {
                                        _currentVideos.removeWhere(
                                          (v) => v['id'] == vid['id'] || v['title'] == vid['title'],
                                        );
                                      }
                                    });
                ...
                ElevatedButton(
                  ...
                  child: Text("Done (${_currentVideos.length} Selected)"),
                ),
              ],
            ),
          );
